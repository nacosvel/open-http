...
class Chainable extends ArrayIterator implements ChainableInterface
{
     ...
     */
    public function __construct(array $array, protected ClientDecoratorInterface $client)
    {
    ...
    {
        // return array_filter($this->getArrayCopy(), static fn($value) => is_string($value));
    ...
    public function offsetGet(mixed $key): ChainableInterface|ClientDecoratorInterface
    {
        if (!$this->offsetExists($key)) {
            $this->offsetSet($key, new self([...$this->retrieve(), $this->normalize($key)], $this->getClient()));
        }
    ...
    {
        return implode($separator, $this->retrieve());
    }
